...
def _subagent_prompt() -> str:
    """Load the research sub-agent prompt with run-time variables inlined."""
    return load_prompt(__file__, "subagent.md").format(
        current_date_and_time=now_str(),
        max_searches=settings.RESEARCH_MAX_SEARCHES_PER_SUBAGENT,
    )


_tools_by_name = {"tavily_search": tavily_search}
